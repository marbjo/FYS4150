...
//   method
    ...
    for ( int i = 0;  i < n; i++){
        ...
        int_gauss += w[i]*w[j]*w[k]*w[l]*w[m]*w[o] * int_function(x[i],x[j],x[k],x[l],x[m],x[o]);
    }}}}}
    }
    ...
    double analytic = 5*atan(1)*4*atan(1)*4/(16*16);

    cout << "Gaussian-Legendre quad = "<< setw(20) << setprecision(15)  << int_gauss << endl;
    cout << "Anaylic solution = " << setw(20) << setprecision(15) << analytic << endl;
    cout << "Error= " << setw(20) << setprecision(15) << fabs(analytic-int_gauss) << endl;
