...
int main(int argc, char *argv[]){
    ...
    //N_
    int N_temp = 32;
    int N_temp_local = N_temp / numprocs;


    double T_start = 2.00;
    double T_end = 2.30; //Has to be 2.31 to include 2.30, because of the logic in the for loop

    //double delta_T = 0.01;
    //int N_temp = round((T_end-T_start)/delta_T / numprocs); //Number of different temperatures

    double delta_T = (T_end-T_start)/N_temp;

    double T_start_local = T_start + my_rank*delta_T*N_temp_local;
    double T_end_local = T_start + (my_rank+1)*delta_T*N_temp_local;
    ...
    cout <<  "==========================" << endl << T_vec << endl << "====================="<< endl;
    arma::vec T_tot(N_temp,arma::fill::zeros); //E(T)
    MPI_Gather(&T_vec, N_temp_local, MPI_DOUBLE, &T_tot, N_temp, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    if(my_rank==0){
        ...
        for(int i=0; i<N_temp; i++){
            expecvalues << setw(15) << setprecision(8) << T_tot[i];
